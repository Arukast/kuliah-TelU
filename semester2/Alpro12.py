...
# Membaca data
file_path = "SF_Air_Traffic_Passenger_Statistics.csv"
...
try:
    ...
    outliers = data_cleaned[
        (data_cleaned["Passenger Count"] < (Q1 - 1.5 * IQR))
        | (data_cleaned["Passenger Count"] > (Q3 + 1.5 * IQR))
    ]
...
try:
    ...
    airline_stats = data_cleaned.groupby("Operating Airline")[
        "Passenger Count"
    ].describe()
...
try:
    ...
    data_cleaned["Activity Period"] = pd.to_datetime(
        data_cleaned["Activity Period"], format="%Y%m"
    )
    ...
    sns.regplot(
        x="Year", y="Passenger Count", data=data_cleaned, scatter=False, color="red"
    )
...
try:
    ...
    # Group By
    grouped_by_airline = (
        data_cleaned.groupby("Operating Airline")["Passenger Count"]
        .sum()
