...
def create_table():
    ...
    cur = conn.cursor()
    cur.execute(
        "CREATE TABLE IF NOT EXISTS tabel_anime(id INTEGER NOT NULL, nama TEXT NOT NULL, anime TEXT NOT NULL, power_level INTEGER NOT NULL, health INTEGER NOT NULL)"
    )
    conn.commit()
    cur.close()
...
def insert_character(id, nama, anime, power_level, health):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO tabel_anime(id, nama, anime, power_level, health) VALUES(?, ?, ?, ?, ?)",
        (id, nama, anime, power_level, health),
    )
    conn.commit()
...
def simulate_battle(character_id, enemy_id):
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE tabel_anime SET health = ? WHERE id = ?", (health_baru, diserang[0])
    )
    conn.commit()
    cur.close()
    ...
    print(
        f"{menyerang[1]} menyerang {diserang[1]}!, {diserang[1]}'s health berkurang menjadi {health_baru}"
    )
